...
public class HashMapTable {
			...
			if (astNode.getData().contains("block")) {
				...
				if (hashArray.get(scopeCounter).containsKey(astNode.getChildren().get(1).getData())) {
					...
					while (addVarDeclToCorrectScope(astNode) == false) {
						// check to make sure that variable is not already
						// declared
						// in the scope
				 ...
				 * astNode.getChildren().get(0).getData()); }else{
				 * scopeCounter--;
				 * 
				 * if(findParentBlock(astNode).equals("block"+scopeCounter)){
				 * System.out.println("AHFAJSFJASF"); hashArray.get(scopeCounter
				...
				// System.out.println(scopeCounter);
				// Look up symbol to the right(astNode.getChildren().get(1)) in
				// the
				// current scope
				// Check types
				...
				if ((checkKeyInCurrScope(astNode, 0) || findVariableInOtherScopeNoTypeCheck(astNode, 0))
					...
					// assigning a variable to a variable, variables have been
					// found
					...
					if (checkTwoVarAssign(astNode, 0, 1)) {
						System.out.println("Variable " + astNode.getChildren().get(0).getData() + " and variable "
								+ astNode.getChildren().get(1).getData()
								+ " have been found and are type compatible! :)");
					} else {
					...
					if (assignTypeCheck(astNode, 1) == 1) {
						...
						// get returns the value of a given key so given the
						// example
						// string a
						// it would return stringWord for the variable a meaning
						// a
						...
						// we know this because if it isn't a string or a
						// boolean it
						// can only be an int. If it were something else the
						// parser
					...
					// Variable not declared in the current scope, so check if
					// it
				...
				if (astNode.getChildren().size() == 1 && (astNode.getChildren().get(0).getChildren().isEmpty())) {
					...
					if (checkKeyInCurrScope(astNode, 0)) {
						// System.out.println("VARIABLE FOUND IN CURRENT
						// SCOPE");
						...
						// Variable not found in current scope and is not a
						// literal
					...
					// iterate through the children of print and find the
					// variables
					// and string literals
					// if theres more than one in the print statement then it is
					// an
					// int expression
					// and the variable will always be at the end

					// System.out.println("You have more than one variable in
					// the
					// print statment: Checki");
					// check if the last child is either an int variable or an
					// int
					// literal
				...
				if (checkKeyInCurrScopeNotExplicit(astNode, 0) && checkKeyInCurrScopeNotExplicit(astNode, 1)) {
					...
					scopeTypeCheckCompVariables(astNode);
				} else if (checkKeyInCurrScopeNotExplicit(astNode, 0)
						&& findVariableInOtherScopeNoTypeCheck(astNode, 1)) {

					scopeTypeCheckCompVariables(astNode);
				} else if (findVariableInOtherScopeNoTypeCheck(astNode, 0)
						&& checkKeyInCurrScopeNotExplicit(astNode, 1)) {
					...
					if (check == "varDecl") {
						// would have found the scope previously so at this
						// point
						// its an error
				...
				if (checkKeyInCurrScope(astNode, 1) || findVariableInOtherScopeNoTypeCheck(astNode, 1)) {
					...
					} else if (checkCompType(astNode)[0].equals("literal")
							&& checkCompType(astNode)[1].equals("literal")) {
	...
	public void checkIntExpression(TreeNode t) {
		...
		if ((!t.getChildren().isEmpty()) && t.getChildren().get(0).getData().matches("\\d")) {
			...
			if (t.getChildren().get(0).getData().startsWith("\"")) {
	...
	public boolean checkTwoVarAssign(TreeNode t, int child1, int child2) {
		...
		if (checkKeyInCurrScope(t, child1)) {
			// System.out.println("Found " +
			// t.getChildren().get(child1).getData() + " in current scope");
		...
		if (checkKeyInCurrScope(t, child2)) {
			// System.out.println("Found " +
			// t.getChildren().get(child2).getData() + " in current scope");
			...
			for (int i = 0; i < scopeCounter - 1; i++) {
				if (hashArray.get(i).containsKey(t.getChildren().get(child).getData())) {
					...
					if (hashArray.get(i).get(t.getChildren().get(child).getData()) == check) {
						System.out.println(
								"TYPE MATCH ON DIFFERENT SCOPED VALUES :) " + t.getChildren().get(child).getData()
										+ " and " + t.getChildren().get(child - 1).getData());
					} else {
